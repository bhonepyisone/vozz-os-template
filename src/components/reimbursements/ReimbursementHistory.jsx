...
export default function ReimbursementHistory() {
  ...
  return (
    <Card title="Your Reimbursement History">
        ...
        {requests.map(req => (
          <div key={req.id} className="p-4 border border-neo-dark/20 rounded-lg flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-gray-600">Amount: {req.amount.toFixed(2)}</p>
              <p className="text-xs text-gray-400">Submitted: {formatDate(req.requestedAt)}</p>
            </div>
